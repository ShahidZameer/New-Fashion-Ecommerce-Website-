...
document.getElementById('col-lg-6').innerHTML = categoriess.map((items)=>{
    ...
    return(  
        ...
        <p class ="heading">${Type}</p>
        <h3 class="Title">${Title}</h3>
        <p class="Price">$${Price2} <span> $${discount}</span></p>
        </div>  
        </div> 
        </div>
        </div>
...
document.getElementById('col-lg-3').innerHTML = shop3.map((items)=>{
    var {image,Price,Title,discount} = items 
    return(
        ` 
        <div class="outfit">   
        <div class="overflow">   
        ...
        <p class="Price">Rs:${Price} <span> 420</span></p> 
        </div>     
...
function clock(){
    ...
    input[0].value= Math.floor(diff / 3600 / 60 / 60  / 24 )
